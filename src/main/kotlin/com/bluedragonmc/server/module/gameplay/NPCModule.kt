...
class NPCModule : GameModule() {
    ...
    fun addNPC(
        ...
        addNPC(
            instance, position, NPC(uuid, customName, skin, entityType, interaction, customNameVisible, lookAtPlayer)
        )
    }
    ...
    class NPC(
        ...
        private val addPlayerPacket: PlayerInfoPacket = PlayerInfoPacket(
            PlayerInfoPacket.Action.ADD_PLAYER, PlayerInfoPacket.AddPlayer(
                uuid, LegacyComponentSerializer.legacySection().serialize(customName), if (skin != null) listOf(
                    PlayerInfoPacket.AddPlayer.Property(
                        "textures", skin.textures(), skin.signature()
                    )
                )
                else emptyList(), GameMode.CREATIVE, 0, customName
            )
        )
        ...
        init {
            ...
            enableFullSkin()
//            entityMeta.isCustomNameVisible = false
...
//                if (::hologram.isInitialized) hologram.remove()
//                hologram = Hologram(instance, spawnPosition.add(0.0, 1.75, 0.0), customName, true)
//            }
...
//        }
    ...
    enum class NPCSkins(val skin: PlayerSkin) {
        ...
        BED_HEAD(
            PlayerSkin(
                "",
